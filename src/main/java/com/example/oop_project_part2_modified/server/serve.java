...
public class serve implements Runnable{
    ...
    private Integer restauratnNO;
    private static int serveCount=0;


    public serve(Socket socket, dataBase d, ArrayList<restaurant> loggedInRestaurant, ArrayList<clientRestaurantPage> rController,HashMap<String,ObjectOutputStream> rPathway,HashMap<String,Integer> order,Integer clientNo,Integer restauratnNO){
        this.socket=socket;
        this.d1=d;
        ...
        this.restauratnNO=restauratnNO;
        serveCount++;
    ...
    public void run(){
        try {
            System.out.println("serve method running "+serveCount);
            System.out.println("serve thread is running ");
            ...
            if (choice.equals("restaurant")) {
                restauratnNO++;
                new ServeRestaurant(socket, d1,oos,ois,loggedInRestaurant,this.rController,this.rPathway);
            }
            else if(choice.equals("client")){
                clientNo++;
                new serveClient(socket,d1,oos,ois,loggedInRestaurant,rPathway,order,clientNo);
            }
